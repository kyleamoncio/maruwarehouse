...
test('Portal blocks order writes unless the V2 backend is in the explicit document-safe version set',()=>{
  const bridge=fs.readFileSync(path.resolve(__dirname,'..','api','sheets.js'),'utf8');
  assert.match(bridge,/ORDER_SAFE_V2_VERSIONS\s*=\s*new Set\([\s\S]*"2026-08-18\.47"\]\)/);
...
test('read-only startup calls have a cold-start-safe timeout budget',()=>{
  ...
  assert.match(bridge,/\["setupPersonalTab", "setupViews", "repairWarehouseFollowupData", "repairWarehousePresentationData", "repairHistoricalPersonalData"\]\.includes\(action\)[\s\S]*"V2",\s*55000/);
